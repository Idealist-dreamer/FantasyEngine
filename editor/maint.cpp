...
class CoreInitSystem : public System {
 public:
  ...
  bool init(Blackboard& bb) override {
  ...
  void save(Blackboard& bb, Archive& ar) override {
    if (bb.has<ContextStorage<GlobalTime>>()) {
      auto& storage = bb.get<ContextStorage<GlobalTime>>();
    ...
    if (bb.has<ContextStorage<GameStats>>()) {
      auto& storage = bb.get<ContextStorage<GameStats>>();
      if (storage.data.valid()) {
  ...
  void load(Blackboard& bb, Archive& ar) override {
    if (!bb.has<ContextStorage<GlobalTime>>()) {
      bb.emplace<ContextStorage<GlobalTime>>();
    }
    if (!bb.has<ContextStorage<GameStats>>()) {
      bb.emplace<ContextStorage<GameStats>>();
    }
    ...
    auto* time = timeStorage.data.get<GlobalTime>();
    auto* stats = statsStorage.data.get<GameStats>();
...
SaveDataSnapshot captureSnapshot(Scene& scene) {
  ...
  auto& bb = scene.blackboard();

  if (bb.has<Registry>()) {
    auto& reg = bb.get<Registry>();
    snapshot.entityCount = static_cast<uint32_t>(
        reg.view<ParticleData>().size() + reg.view<Transform>().size());
    snapshot.transformCount =
        static_cast<uint32_t>(reg.view<Transform>().size());
    snapshot.velocityCount = static_cast<uint32_t>(reg.view<Velocity>().size());
    snapshot.boidStateCount =
        static_cast<uint32_t>(reg.view<BoidState>().size());
    snapshot.renderStateCount =
        static_cast<uint32_t>(reg.view<RenderState>().size());
    snapshot.particleCount =
        static_cast<uint32_t>(reg.view<ParticleData>().size());
