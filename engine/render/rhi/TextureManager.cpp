...
ManagedTexture::ManagedTexture(const wstring& FileName) : m_MapKey(FileName), m_IsValid(false), m_IsLoading(true), m_ReferenceCount(0) {
...
void ManagedTexture::CreateFromMemory(ByteArray ba, eDefaultTexture fallback, bool forceSRGB) {
  if (ba->size() == 0) {
    ...
    if (SUCCEEDED(CreateDDSTextureFromMemory(g_Device, (const uint8_t*)ba->data(), ba->size(), 0, forceSRGB, m_pResource.GetAddressOf(),
      ...
      g_Device->CopyDescriptorsSimple(1, m_hCpuDescriptorHandle, GetDefaultTexture(fallback), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    }
  }
