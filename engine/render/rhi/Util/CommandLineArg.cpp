...
bool GetInteger(const wchar_t* key, uint32_t& value) {
  return Lookup(key, [&value](std::wstring& val) { value = std::stoi(val); });
}

bool GetFloat(const wchar_t* key, float& value) {
  return Lookup(key, [&value](std::wstring& val) { value = (float)_wtof(val.c_str()); });
}

bool GetString(const wchar_t* key, std::wstring& value) {
  return Lookup(key, [&value](std::wstring& val) { value = val; });
}
}  // namespace CommandLineArgs