...
#undef CreatePSO
  ...
  FinalDispatchIndirectArgs.Create(L"ParticleEffectManager::FinalDispatchIndirectArgs", 1, sizeof(D3D12_DISPATCH_ARGUMENTS),
                                   InitialDispatchIndirectArgs);
  ...
  ASSERT_SUCCEEDED(
      g_Device->CreateCommittedResource(&HeapProps, D3D12_HEAP_FLAG_NONE, &TexDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, MY_IID_PPV_ARGS(&tex)));
...
void ParticleEffectManager::Render(CommandContext& Context, const Camera& Camera, ColorBuffer& ColorTarget, DepthBuffer& DepthTarget,
