...
NumVar BloomUpsampleFactor("Graphics/Bloom/Scatter", 0.65f, 0.0f, 1.0f,
                           0.05f);                              // Controls the "focus" of the blur.  High values spread out more causing a haze.
...
#undef CreatePSO
  ...
  __declspec(align(16)) float initExposure[] = {Exposure,
                                                1.0f / Exposure,
                                                Exposure,
                                                0.0f,
                                                kInitialMinLog,
...
void PostEffects::GenerateBloom(ComputeContext& Context) {
  ...
  RE_ASSERT(kBloomWidth % 16 == 0 && kBloomHeight % 16 == 0, "Bloom buffer dimensions must be multiples of 16");
...
void PostEffects::UpdateExposure(ComputeContext& Context) {
  ...
  __declspec(align(16)) struct {
    ...
    float MaxExposure;
    uint32_t PixelCount;
  } constants = {TargetLuminance, AdaptationRate, MinExposure, MaxExposure, g_LumaLR.GetWidth() * g_LumaLR.GetHeight()};
