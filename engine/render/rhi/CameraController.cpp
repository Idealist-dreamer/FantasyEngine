...
FlyingFPSCamera::FlyingFPSCamera(Camera& camera, Vector3 sceneUp) : CameraController(camera) {
  m_SceneUp = Normalize(sceneUp);
  m_SceneNorth = Normalize(Cross(m_SceneUp, Vector3(kXUnitVector)));
  m_SceneEast = Cross(m_SceneNorth, m_SceneUp);
  ...
  m_CurrentPitch = Sin(Dot(camera.GetForwardVec(), m_SceneUp));

  Vector3 forward = Normalize(Cross(m_SceneUp, camera.GetRightVec()));
  m_CurrentHeading = ATan2(-Dot(forward, m_SceneEast), Dot(forward, m_SceneNorth));
...
void FlyingFPSCamera::Update(float deltaTime) {
  ...
  float forward = m_MoveSpeed * speedScale *
                  (GameInput::GetTimeCorrectedAnalogInput(GameInput::kAnalogLeftStickY) +
                   (GameInput::IsPressed(GameInput::kKey_w) ? deltaTime : 0.0f) + (GameInput::IsPressed(GameInput::kKey_s) ? -deltaTime : 0.0f));
  float strafe = m_StrafeSpeed * speedScale *
                 (GameInput::GetTimeCorrectedAnalogInput(GameInput::kAnalogLeftStickX) +
                  (GameInput::IsPressed(GameInput::kKey_d) ? deltaTime : 0.0f) + (GameInput::IsPressed(GameInput::kKey_a) ? -deltaTime : 0.0f));
  float ascent = m_StrafeSpeed * speedScale *
                 (GameInput::GetTimeCorrectedAnalogInput(GameInput::kAnalogRightTrigger) -
                  ...
                  (GameInput::IsPressed(GameInput::kKey_e) ? deltaTime : 0.0f) + (GameInput::IsPressed(GameInput::kKey_q) ? -deltaTime : 0.0f));
  ...
  Matrix3 orientation =
      Matrix3(m_SceneEast, m_SceneUp, -m_SceneNorth) * Matrix3::MakeYRotation(m_CurrentHeading) * Matrix3::MakeXRotation(m_CurrentPitch);
...
OrbitCamera::OrbitCamera(Camera& camera, Math::BoundingSphere focus, Vector3 sceneUp) : CameraController(camera) {
  m_ModelBounds = focus;
  m_SceneUp = Normalize(sceneUp);
