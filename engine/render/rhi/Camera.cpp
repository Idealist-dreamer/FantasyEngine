...
void BaseCamera::SetLookDirection(Vector3 forward, Vector3 up) {
  ...
  m_CameraToScene.SetRotation(Quaternion(m_Basis));
}

void BaseCamera::Update() {
  m_PreviousViewProjMatrix = m_ViewProjMatrix;

  m_ViewMatrix = Matrix4(~m_CameraToScene);
  m_ViewProjMatrix = m_ProjMatrix * m_ViewMatrix;
  m_ReprojectMatrix = m_PreviousViewProjMatrix * Invert(GetViewProjMatrix());

  m_FrustumVS = Frustum(m_ProjMatrix);
  m_FrustumWS = m_CameraToScene * m_FrustumVS;
}
...
void Camera::UpdateProjMatrix(void) {
  ...
  SetProjMatrix(Matrix4(Vector4(X, 0.0f, 0.0f, 0.0f), Vector4(0.0f, Y, 0.0f, 0.0f), Vector4(0.0f, 0.0f, Q1, -1.0f), Vector4(0.0f, 0.0f, Q2, 0.0f)));
}
