...
void FXAA::Render(ComputeContext& Context, bool bUsePreComputedLuma) {
  ...
  D3D12_CPU_DESCRIPTOR_HANDLE Pass1UAVs[] = {WorkCounters.GetUAV(), g_FXAAWorkQueue.GetUAV(), g_FXAAColorQueue.GetUAV(), g_LumaBuffer.GetUAV()};

  D3D12_CPU_DESCRIPTOR_HANDLE Pass1SRVs[] = {Target.GetSRV(), g_LumaBuffer.GetSRV()};
