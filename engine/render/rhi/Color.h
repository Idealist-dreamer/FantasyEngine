...
FE_FINLINE Color Max(Color a, Color b) {
  return Color(XMVectorMax(a, b));
}
FE_FINLINE Color Min(Color a, Color b) {
  return Color(XMVectorMin(a, b));
}
FE_FINLINE Color Clamp(Color x, Color a, Color b) {
  return Color(XMVectorClamp(x, a, b));
}

FE_FINLINE Color::Color(FXMVECTOR vec) {
  m_value.v = vec;
}

FE_FINLINE Color::Color(const XMVECTORF32& vec) {
  m_value = vec;
}
...
FE_FINLINE Color::Color(uint16_t r, uint16_t g, uint16_t b, uint16_t a, uint16_t bitDepth) {
  m_value.v = XMVectorScale(XMVectorSet(r, g, b, a), 1.0f / ((1 << bitDepth) - 1));
}

FE_FINLINE Color::Color(uint32_t u32) {
...
FE_FINLINE Color Color::ToSRGB(void) const {
...
FE_FINLINE Color Color::FromSRGB(void) const {
  XMVECTOR T = XMVectorSaturate(m_value);
...
FE_FINLINE Color Color::ToREC709(void) const {
  XMVECTOR T = XMVectorSaturate(m_value);
...
FE_FINLINE Color Color::FromREC709(void) const {
  XMVECTOR T = XMVectorSaturate(m_value);
...
FE_FINLINE uint32_t Color::R10G10B10A2(void) const {
...
FE_FINLINE uint32_t Color::R8G8B8A8(void) const {
