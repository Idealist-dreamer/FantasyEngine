...
class ColorBuffer : public PixelBuffer {
 public:
  ...
  void Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t NumMips, DXGI_FORMAT Format, EsramAllocator& Allocator);
  ...
  // Create a color buffer.  Memory will be allocated in ESRAM (on Xbox One).  On Windows,
  // this functions the same as Create() without a video address.
  void CreateArray(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t ArrayCount, DXGI_FORMAT Format, EsramAllocator& Allocator);
 ...
 protected:
  ...
  static FE_FINLINE uint32_t ComputeNumMips(uint32_t Width, uint32_t Height) {
    uint32_t HighBit;
