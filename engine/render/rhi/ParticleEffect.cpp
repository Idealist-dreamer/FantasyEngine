...
FE_FINLINE static Color RandColor(Color c0, Color c1) {
  // We might want to find min and max of each channel rather than assuming c0 <= c1
  return Color(s_RNG.NextFloat(c0.R(), c1.R()), s_RNG.NextFloat(c0.G(), c1.G()), s_RNG.NextFloat(c0.B(), c1.B()), s_RNG.NextFloat(c0.A(), c1.A()));
}

FE_FINLINE static XMFLOAT3 RandSpread(const XMFLOAT3& s) {
  // We might want to find min and max of each channel rather than assuming c0 <= c1
  return XMFLOAT3(s_RNG.NextFloat(-s.x, s.x), s_RNG.NextFloat(-s.y, s.y), s_RNG.NextFloat(-s.z, s.z));
}
...
void ParticleEffect::LoadDeviceResources(ID3D12Device* device) {
  ...
  m_RandomStateBuffer.Create(L"ParticleSystem::SpawnDataBuffer", m_EffectProperties.EmitProperties.MaxParticles, sizeof(ParticleSpawnData),
                             pSpawnData);
  _freea(pSpawnData);
