...
void BoolVar::SetValue(FILE* file, const std::string& setting) {
  ...
  m_Flag = (0 == _stricmp(valstr, "1") || 0 == _stricmp(valstr, "on") || 0 == _stricmp(valstr, "yes") || 0 == _stricmp(valstr, "true"));
}
...
CallbackTrigger::CallbackTrigger(const std::string& path, std::function<void(void*)> callback, void* args) : EngineVar(path) {
  m_Callback = callback;
  m_Arguments = args;
  m_BangDisplay = 0;
}
...
void EngineTuning::Display(GraphicsContext& Context, float x, float y, float w, float h) {
  ...
  Context.SetScissor((uint32_t)Floor(x * hScale), (uint32_t)Floor(y * vScale), (uint32_t)Ceiling((x + w) * hScale),
                     (uint32_t)Ceiling((y + h) * vScale));
