...
FE_FINLINE XMVECTOR SplatZero() {
  return XMVectorZero();
}

#if !defined(_XM_NO_INTRINSICS_) && defined(_XM_SSE_INTRINSICS_)

FE_FINLINE XMVECTOR SplatOne(XMVECTOR zero = SplatZero()) {
...
FE_FINLINE XMVECTOR CreateXUnitVector(XMVECTOR one = SplatOne()) {
  return _mm_insert_ps(one, one, 0x0E);
}
FE_FINLINE XMVECTOR CreateYUnitVector(XMVECTOR one = SplatOne()) {
  return _mm_insert_ps(one, one, 0x0D);
}
FE_FINLINE XMVECTOR CreateZUnitVector(XMVECTOR one = SplatOne()) {
  return _mm_insert_ps(one, one, 0x0B);
}
FE_FINLINE XMVECTOR CreateWUnitVector(XMVECTOR one = SplatOne()) {
  return _mm_insert_ps(one, one, 0x07);
}
FE_FINLINE XMVECTOR SetWToZero(FXMVECTOR vec) {
  return _mm_insert_ps(vec, vec, 0x08);
}
FE_FINLINE XMVECTOR SetWToOne(FXMVECTOR vec) {
...
FE_FINLINE XMVECTOR SplatOne() {
  return XMVectorSplatOne();
}
FE_FINLINE XMVECTOR CreateXUnitVector() {
  return g_XMIdentityR0;
}
FE_FINLINE XMVECTOR CreateYUnitVector() {
  return g_XMIdentityR1;
}
FE_FINLINE XMVECTOR CreateZUnitVector() {
  return g_XMIdentityR2;
}
FE_FINLINE XMVECTOR CreateWUnitVector() {
