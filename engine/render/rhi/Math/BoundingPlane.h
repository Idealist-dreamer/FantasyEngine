...
class BoundingPlane {
 public:
  ...
  FE_FINLINE operator Vector4() const { return m_repr; }
  ...
  friend BoundingPlane operator*(const Matrix4& mat, BoundingPlane plane) { return BoundingPlane(Transpose(Invert(mat)) * plane.m_repr); }
...
//
FE_FINLINE BoundingPlane::BoundingPlane(Vector3 pointOnPlane, Vector3 normalToPlane) {
...
//
FE_FINLINE BoundingPlane PlaneFromPointsCCW(Vector3 A, Vector3 B, Vector3 C) {
