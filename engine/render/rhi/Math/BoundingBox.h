...
class AxisAlignedBox {
 public:
  ...
  AxisAlignedBox Union(const AxisAlignedBox& box) { return AxisAlignedBox(Min(m_min, box.m_min), Max(m_max, box.m_max)); }
...
FE_FINLINE OrientedBox operator*(const UniformTransform& xform, const OrientedBox& obb) {
  return AffineTransform(xform) * obb;
}

FE_FINLINE OrientedBox operator*(const UniformTransform& xform, const AxisAlignedBox& aabb) {
