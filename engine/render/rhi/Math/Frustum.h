...
class Frustum {
 public:
  ...
  enum CornerID { kNearLowerLeft, kNearUpperLeft, kNearLowerRight, kNearUpperRight, kFarLowerLeft, kFarUpperLeft, kFarLowerRight, kFarUpperRight };
...
//=======================================================================================================
// FE_FINLINE implementations
//

FE_FINLINE bool Frustum::IntersectSphere(BoundingSphere sphere) const {
...
FE_FINLINE bool Frustum::IntersectBoundingBox(const AxisAlignedBox& aabb) const {
  for (int i = 0; i < 6; ++i) {
...
FE_FINLINE Frustum operator*(const OrthogonalTransform& xform, const Frustum& frustum) {
  Frustum result;
...
FE_FINLINE Frustum operator*(const AffineTransform& xform, const Frustum& frustum) {
  Frustum result;
...
FE_FINLINE Frustum operator*(const Matrix4& mtx, const Frustum& frustum) {
  Frustum result;
