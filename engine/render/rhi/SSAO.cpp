...
ComputePSO s_BlurUpsampleBlend[2] = {{L"SSAO: Blur Upsample Blend Low Q. CS"},
                                     {L"SSAO: Blur Upsample Blend High Q. CS"}};  // Blend the upsampled result with the next higher resolution
ComputePSO s_BlurUpsampleFinal[2] = {{L"SSAO: Blur Upsample Final Low Q. CS"},
                                     {L"SSAO: Blur Upsample Final High Q. CS"}};  // Don't blend the result, just upsample it
...
void BlurAndUpsample(ComputeContext& Context, ColorBuffer& Destination, ColorBuffer& HiResDepth, ColorBuffer& LoResDepth, ColorBuffer* InterleavedAO,
                     ColorBuffer* HighQualityAO, ColorBuffer* HiResAO) {
  ...
  __declspec(align(16)) float cbData[] = {1.0f / LoWidth,     1.0f / LoHeight,          1.0f / HiWidth, 1.0f / HiHeight,
...
void SSAO::Render(GraphicsContext& GfxContext, const float* ProjMat, float NearClipDist, float FarClipDist) {
...

  {
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE DownsizeUAVs[5] = {LinearDepth.GetUAV(), g_DepthDownsize1.GetUAV(), g_DepthTiled1.GetUAV(), g_DepthDownsize2.GetUAV(),
    ...
    if (HierarchyDepth > 2) {
      ...
      D3D12_CPU_DESCRIPTOR_HANDLE DownsizeAgainUAVs[4] = {g_DepthDownsize3.GetUAV(), g_DepthTiled3.GetUAV(), g_DepthDownsize4.GetUAV(),
