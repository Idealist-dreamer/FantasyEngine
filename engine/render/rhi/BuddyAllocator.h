...
class BuddyAllocator {
 public:
  BuddyAllocator(kBuddyAllocationStrategy allocationStrategy, D3D12_HEAP_TYPE heapType, size_t maxBlockSize,
                 size_t minBlockSize = MIN_PLACED_BUFFER_SIZE, size_t baseOffset = 0);
  ...
  inline bool IsOwner(const BuddyBlock& block) { return block.GetOffset() >= m_baseOffset && block.GetSize() <= m_maxBlockSize; }
 ...
 private:
  ...
  inline size_t SizeToUnitSize(size_t size) const { return (size + (m_minBlockSize - 1)) / m_minBlockSize; }

  inline UINT UnitSizeToOrder(size_t size) const {
    return Math::Log2(size);  // Log2 rounds up fractions to next whole value
  }

  inline size_t GetBuddyOffset(const size_t& offset, const size_t& size) { return offset ^ size; }
