...
class ShadowCamera : public Math::BaseCamera {
 public:
  ...
  void UpdateMatrix(Math::Vector3 LightDirection,  // Direction parallel to light, in direction of travel
                    Math::Vector3 ShadowCenter,    // Center location on far bounding plane of shadowed region
                    Math::Vector3 ShadowBounds,    // Width, height, and depth in scene space represented by the shadow buffer
  ...
  // Used to transform scene space to texture space for shadow sampling
