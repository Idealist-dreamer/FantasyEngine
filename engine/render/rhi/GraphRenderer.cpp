...
class PerfGraph {
 ...
 public:
  ...
  static void RenderGraph(GraphicsContext& Context, uint32_t vertexCount, D3D12_VIEWPORT& viewport, uint32_t debugVarCount, float topMargin);

  void RenderGraph(GraphicsContext& Context, uint32_t vertexCount, D3D12_VIEWPORT& viewport, uint32_t debugVarCount, float topMargin, float MaxValue,
                   uint32_t frameID);

  void RenderGraph(GraphicsContext& Context, uint32_t vertexCount, D3D12_VIEWPORT& viewport, uint32_t debugVarCount, float topMargin,
...
class GraphVector {
 public:
  GraphVector(uint32_t MaxActiveGraphs, uint32_t DebugVarCount)
      : m_MaxActiveGraphs(MaxActiveGraphs),
        m_ActiveGraphs(0),
        m_DebugVarCount(DebugVarCount),
...
static void DrawGraphHeaders(TextContext& Text, float leftMargin, float topMargin, float offsetY, float graphHeight, float* MinArray, float* MaxArray,
                             float* PresetMaxArray, bool GlobalScale, uint32_t numDebugVar, std::string graphTitles[]);
...
void DrawGraphHeaders(TextContext& Text, float leftMargin, float topMargin, float offsetY, float graphHeight, float* MinArray, float* MaxArray,
                      float* PresetMaxArray, bool GlobalScale, uint32_t numDebugVar, std::string graphTitles[]) {
...
void GraphRenderer::RenderGraphs(GraphicsContext& Context, GraphType Type) {
  if (Type == GraphType::Global && GlobalGraphs.Size() == 0 || Type == GraphType::Profile && ProfileGraphs.Size() == 0) {
  ...
  if (Type == GraphType::Profile && ProfileGraphs.GetActiveGraphCount() > 0) {
    ...
    DrawGraphHeaders(Text, (viewport.TopLeftX), blankSpace, 0.0f, (viewport.Height + blankSpace), ProfileGraphs.GetMin(), ProfileGraphs.GetMax(),
                     ProfileGraphs.GetPresetMax(), false, PROFILE_DEBUG_VAR_COUNT, graphTitles);
