...
class Allocator {
 ...
 public:
  ...
  FE_FINLINE static void* reallocn(void* p, size_t count,
                                   size_t newSize) noexcept;
  FE_FINLINE static bool expand(void* p, size_t newSize) noexcept;
  FE_FINLINE static void* malloc_aligned(size_t size,
                                         size_t alignment) noexcept;
  FE_FINLINE static void* zalloc_aligned(size_t size,
                                         size_t alignment) noexcept;
  FE_FINLINE static void* calloc_aligned(size_t count, size_t size,
                                         size_t alignment) noexcept;
  FE_FINLINE static void* realloc_aligned(void* p, size_t newsize,
                                          size_t alignment) noexcept;

  FE_FINLINE static void* malloc_aligned_at(size_t size, size_t alignment,
                                            size_t offset) noexcept;
  FE_FINLINE static void* zalloc_aligned_at(size_t size, size_t alignment,
                                            size_t offset) noexcept;
  FE_FINLINE static void* calloc_aligned_at(size_t count, size_t size,
                                            size_t alignment,
                                            size_t offset) noexcept;
  FE_FINLINE static void* realloc_aligned_at(void* p, size_t newsize,
                                             size_t alignment,
                                             size_t offset) noexcept;
  ...
  FE_FINLINE static T* malloc_type_array(size_t count) noexcept {
    ...
    void* p = (alignment > kDefaultAlignment)
                  ? malloc_aligned(count * sizeof(T), alignment)
  ...
  FE_FINLINE static T* zalloc_type_array(size_t count) noexcept {
    size_t alignment = alignof(T);
    void* p = (alignment > kDefaultAlignment)
                  ? zalloc_aligned(count * sizeof(T), alignment)
                  : zalloc(count * sizeof(T));
    return static_cast<T*>(p);
  }
  ...
  FE_FINLINE static T* create(Args&&... args) {
    size_t alignment = alignof(T);
    void* p = (alignment > kDefaultAlignment)
                  ? malloc_aligned(sizeof(T), alignment)
                  : malloc(sizeof(T));
    if (!p) return nullptr;
